...
if [[ $? == 0 ]]; then
    ...
    NEWID=$(curl -s https://api.modpacks.ch/public/modpack/$(jq ".parent" version.json) | jq ".versions[] | select(.id > $(jq '.id' version.json)) | select(.type | ascii_downcase == \"$TYPE\")" | jq ".id" | sort -n | tail -n1)
    if [[ "$NEWID" == "" ]]; then
		...
		echo -n "Are you sure you want to update to version $NEW_VERSION (y/N) ?"
		read INSTALL
